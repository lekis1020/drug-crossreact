import { oncologyDatabase } from './oncologyDatabase';
import type { OncologyEdgeData, OncologyGraphState, OncologyInfo, OncologyNodeData } from '../types';

const GROUP_COLORS: string[] = ['#38bdf8', '#a78bfa', '#f59e0b', '#22c55e', '#f97316', '#e879f9'];

const RISK_PRIORITY: Record<OncologyEdgeData['crossReactivity'], number> = {
  high: 4,
  disputed: 3,
  moderate: 2,
  low: 1,
...
function toRisk(value: string): OncologyEdgeData['crossReactivity'] | null {
  const key = value.toLowerCase().trim();
  if (key === 'high' || key === 'disputed' || key === 'moderate' || key === 'low') return key;
  return null;
}

let cache: OncologyGraphState | null = null;

export function buildOncologyGraphElements(): OncologyGraphState {
  if (cache) return cache;

  const groupColorMap = new Map(
    oncologyDatabase.agent_groups.map((group, index) => [group.group_id, GROUP_COLORS[index % GROUP_COLORS.length]]),
  );

  const nodeParents: Record<string, string> = {};
  const parentNodes = oncologyDatabase.agent_groups.map((group) => ({
    id: `group-${group.group_id}`,
    label: group.name,
    isGroup: true,
  }));

  for (const group of oncologyDatabase.agent_groups) {
    for (const agentId of group.agents) {
      nodeParents[agentId] = `group-${group.group_id}`;
    }
  }

  const nodes: OncologyNodeData[] = oncologyDatabase.agents.map((agent) => {
    const group = oncologyDatabase.agent_groups.find((g) => g.agents.includes(agent.id));
    return {
      id: agent.id,
      label: agent.label,
  ...
  const edgeMap = new Map<string, OncologyEdgeData>();

  const upsertEdge = (edge: OncologyEdgeData) => {
  ...
  const templateRule = oncologyDatabase.risk_rules[0];
  ...
  for (const pair of oncologyDatabase.literature_confirmed_pairs) {
...
export function getCrossReactiveAgents(selectedId: string): OncologyInfo[] {
  const { nodes, edges } = buildOncologyGraphElements();
  const nodeMap = new Map(nodes.map((node) => [node.id, node]));
  const result: OncologyInfo[] = [];
  ...
  const riskOrder: Record<OncologyEdgeData['crossReactivity'], number> = {
    high: 0,
    disputed: 1,
    moderate: 2,
    low: 3,
...
export function getNoKnownSignalAlternatives(selectedId: string): OncologyNodeData[] {
  const { nodes, edges } = buildOncologyGraphElements();
...
export function getOncologyGroupLabel(groupId: string): string {
  return groupMapFallback(groupId);
}

function groupMapFallback(groupId: string): string {
  const match = oncologyDatabase.agent_groups.find((group) => group.group_id === groupId);
