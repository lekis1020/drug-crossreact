import { buildOncologyGraphElements, getCrossReactiveAgents, getNoKnownSignalAlternatives } from '../data/oncologyGraphData';
import { RiskBadge } from '../../components/RiskBadge';
import type { OncologyNodeData } from '../types';
import { COLORS, EFFECTS } from '../../styles/design-tokens';

interface OncologySidePanelProps {
...
export function OncologySidePanel({ selectedAgentId }: OncologySidePanelProps) {
  if (!selectedAgentId) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-slate-950/20">
        ...
        <p className="text-sm text-slate-500 max-w-[200px]">
          Select an anticancer drug from the graph to analyze cross-reactivity evidence.
        </p>
      </div>
    );
  }
  ...
  const alternativesByGroup: Record<string, OncologyNodeData[]> = {};
    ...
    <div className="h-full overflow-y-auto custom-scrollbar" style={{ background: COLORS.bg.panel }}>
      ...
      <div className="p-6 border-b border-white/5">
        ...
        <div className="bg-white/5 rounded-xl p-4 border border-white/10 shadow-inner">
          <div className="grid grid-cols-2 gap-y-3 gap-x-4">
            ...
            <div>
              <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">Class</p>
              <p className="text-sm text-slate-200">{agent.drugClass}</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">Formulation</p>
              <p className="text-sm text-slate-200">{agent.formulation}</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">Mechanism</p>
              <p className="text-sm text-slate-200">{agent.mechanism}</p>
            </div>
          </div>
        </div>
      ...
      <div className="p-6">
        ...
        <p className="text-[10px] text-slate-500 mb-5 leading-normal bg-emerald-500/5 p-2 rounded border border-emerald-500/10">
          ⚠️ <span className="text-emerald-300/80 font-semibold text-[11px]">No known signal ≠ guaranteed safety.</span> Final choice should consider regimen intent, reaction phenotype, and oncology-allergy protocol guidance.
        </p>
