...
import { searchOncologyNodes } from '../../data/searchAliases';

interface OncologySearchBarProps {
  onSearch: (agentId: string) => void;
  selectedAgent: string | null;
}

export function OncologySearchBar({ onSearch, selectedAgent }: OncologySearchBarProps) {
  const [query, setQuery] = useState('');
  const [isOpen, setIsOpen] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);

  const { nodes } = buildOncologyGraphElements();
  const matches = query.length > 0 ? searchOncologyNodes(nodes, query) : [];
    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search oncology ingredient or brand..."
