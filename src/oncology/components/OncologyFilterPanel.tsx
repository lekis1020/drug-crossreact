...
interface OncologyFilterPanelProps {
  filters: OncologyFilterState;
  onFiltersChange: (filters: OncologyFilterState) => void;
}
...
export function OncologyFilterPanel({ filters, onFiltersChange }: OncologyFilterPanelProps) {
  ...
  const groupEntries = useMemo(
    () => oncologyDatabase.agent_groups.map((group) => ({ id: group.group_id, label: group.name })),
    [],
  );
  ...
  const toggleRisk = (risk: keyof OncologyFilterState['risks']) => {
    onFiltersChange({
      ...filters,
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="space-y-3 max-h-[70vh] overflow-y-auto custom-scrollbar pr-1">
            {/* Oncology Groups Section */}
            <div className="rounded-xl border border-white/5 bg-white/[0.02] p-2">
              <SectionHeader
                title="Oncology Groups"
